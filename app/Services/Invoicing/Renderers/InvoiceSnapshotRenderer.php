...
/**
 * The shared layout of every invoice snapshot renderer: a standalone, print-optimized HTML document for an 80mm
 * thermal roll (fixed narrow width, monospace, no colour, no scripts). Every value comes from the snapshot and is
 * escaped -- product names, buyer and seller details are typed by people, and the result is displayed in a browser.
 *
 * Deliberately says nothing about legal wording (no "official receipt" or "valid for input tax" text): which
 * statements a BIR-registered invoice must carry is a matter for the business or legal owner (BIR-006), not
 * something to invent here. The seller block prints what the snapshot recorded and omits what it did not.
 *
 * A schema version adds to the layout only through the four hooks below, which print nothing here, so a renderer
 * for an older version keeps producing exactly the bytes it always did (ADR-006).
 */
abstract class InvoiceSnapshotRenderer
{
    ...
    public function render(array $snapshot, ?CarbonInterface $reprintedAt = null): string
    {
        ...
        array_push($html, ...$this->machineBlock($snapshot));
        ...
        array_push($html, ...$this->beneficiaryBlock($snapshot));
    ...
    /**
     * Machine identification lines, printed in the header. Nothing in the base layout.
     *
     * @param  array<string, mixed>  $snapshot
     * @return list<string>
     */
    ...
    /**
     * Buyer-entitlement lines (Senior Citizen, PWD and the like), printed after the buyer block.
     *
     * @param  array<string, mixed>  $snapshot
     * @return list<string>
     */
    ...
    /**
     * Payment lines, printed under the total. A block that prints anything ends with its own separator.
     *
     * @param  array<string, mixed>  $snapshot
     * @return list<string>
     */
    ...
    /**
     * The registration and accreditation footer. A block that prints anything begins with its own separator.
     *
     * @param  array<string, mixed>  $snapshot
     * @return list<string>
     */
    ...
    protected function row(string $left, string $right): string
    {
        return '<div class="row"><span>'.$this->text($left).'</span><span>'.$right.'</span></div>';
    }

    /** @param  array<string, mixed>  $snapshot */
    protected function has(array $snapshot, string $key): bool
    {
    ...
    protected function positive(string $money): bool
    {
    ...
    protected function dateTime(?string $iso): string
    {
        if ($iso === null || $iso === '') {
            return '';
        }
