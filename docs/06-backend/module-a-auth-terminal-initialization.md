...
**RULINGS CLOSED. A0 CLOSED. A1 (human authentication/session foundation) GOVERNANCE-SEALED**, after two baseline reconstructions. **A2 (authorization/capability foundation) SEALED. A3 (terminal enrollment/credential verification) GOVERNANCE-SEALED**, after a third and then a fourth baseline reconstruction (see §19a and §19b). A4 through A6 have not started — no authoritative User+Terminal+Store composition, user-management endpoints, or Stage 6C HTTP integration exists yet.
