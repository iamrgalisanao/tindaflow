...
**RULINGS CLOSED. A0 CLOSED. A1 (human authentication/session foundation) GOVERNANCE-SEALED**, after two baseline reconstructions. **A2 (authorization/capability foundation) IMPLEMENTED. A3 (terminal enrollment/credential verification) IMPLEMENTED**, with one error-code decision flagged for the owner (see §19). A4 through A6 have not started — no authoritative User+Terminal+Store composition, user-management endpoints, or Stage 6C HTTP integration exists yet.
...
**A3** implemented ADR-011's terminal enrollment/credential trust boundary — see §19 for the full implementation summary, including a real project-wide PostgreSQL session-timezone bug found and fixed along the way. One decision is flagged for the owner rather than resolved unilaterally: `terminalEnroll`'s 409 outcome has no error-catalog.md code (`ENROLLMENT_TOKEN_INVALID` implemented as the proposed candidate, not yet catalogued). No Module A baseline/tag has been created; per the same discipline used throughout this project, that happens only on explicit owner review once A1–A6 are complete.
