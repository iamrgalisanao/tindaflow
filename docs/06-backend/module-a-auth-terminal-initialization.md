...
**RULINGS CLOSED. A0 CLOSED. A1 (human authentication/session foundation) GOVERNANCE-SEALED**, after two baseline reconstructions. **A2 (authorization/capability foundation) SEALED. A3 (terminal enrollment/credential verification) GOVERNANCE-SEALED**, after a third and then a fourth baseline reconstruction (see §19a and §19b). **A4 (authoritative User+Terminal+Store request-context composition) IMPLEMENTED** — see §20; no baseline reconstruction required. **A5 (HTTP integration test matrix) IMPLEMENTED** — see §21. **A6 (Stage 6C checkout integration) IMPLEMENTED** — see §22; all §13 matrix items, including the two A5 deferred, are now proven against the real `SaleController`. **The full A0–A6 Module A implementation sequence (§15) is complete.** All nine of §16's Stage 6C unblock criteria are now met; Stage 6C's own freeze as `stage-6c-baseline` remains an explicit owner decision, per this project's standing practice of never self-sealing a stage.
...
- `TerminalSummary` missing `credential_issued_at`/`revoked_at`: **deferred**, not blocking Module A or Stage 6C. Revisit as a Stage 4 amendment when back-office terminal-management UI is built (Stage 7). — **`revoked_at` REVISITED AND RESOLVED (2026-09-26)**: that UI now exists (`/admin/terminals`, with the revoke action), and it cannot show which terminals are revoked without this field — nothing else carries the fact, since revocation deliberately leaves `status` alone (see the last bullet of this ruling). `revoked_at` is now returned by `TerminalSummaryResource` as a **forward-committed** addition and added to the batched Stage 4 amendment below; the frozen `openapi.yaml` text is not edited ahead of that reconstruction pass. Additive and backwards-compatible — no existing field changed meaning. `credential_issued_at` stays deferred: still no consumer.
...
3. **(added 2026-09-26)** Add `revoked_at: {type: string, format: date-time, nullable: true}` to `components/schemas/TerminalSummary`. Already returned by `TerminalSummaryResource` and covered by `TerminalEnrollmentTest::test_revoked_at_is_exposed_and_status_is_left_alone`, which also pins that revocation leaves `status` untouched and that `credential_hash`/`credential_issued_at` stay out of the shape. Until this pass runs, the implementation is ahead of the frozen document by exactly this one optional response field — the same forward-commitment pattern Stages 26/29/32/34/35/36 already use.
...
## 22. A6 implementation summary (Stage 6C checkout HTTP integration) — Module A complete

**Scope delivered**: exactly §15's A6 definition — `SaleController`/`SaleFinalizeRequest`/route, wiring A4's `PosRequestContext` into `CheckoutService::finalize()` precisely as `stage-6c-sale-finalization.md`'s "Authentication boundary" section already specified, registered under `routes/web.php` (the `web` group, per §14 Ruling 5) — never the default stateless `api` group. No change to `CheckoutService` itself, no new business/domain logic; Stage 6C's already-VERIFIED service layer is exercised as-is.

**Controller**: `SaleController::finalize()` extracts the `Idempotency-Key` header directly (throwing `IdempotencyKeyRequiredException` if absent, 400), reads `PosRequestContext` from the `pos_context` request attribute `ComposeAuthoritativeContext` already attaches, and calls `CheckoutService::finalize($context->terminal->id, $context->user->id, $idempotencyKey, $request->validated())`. `terminal_id`/`cashier_id`/`user_id` are never read from the request body anywhere in this path — proven directly (`test_request_body_cannot_spoof_terminal_or_cashier_identity`), not merely asserted by the code's shape.

**Middleware chain**, matching `terminalCurrent`'s already-established pattern exactly: `auth` → `EnsureUserIsActive` → `ResolveTerminalContext` → `ComposeAuthoritativeContext`. No `can:` capability gate — `openapi.yaml`'s `saleFinalize` declares no `x-capability`, confirmed by direct inspection, so any authenticated user on a store-coherent, enrolled terminal may attempt checkout; `CheckoutService`'s own OPEN-shift/cashier-match check is the remaining, already-implemented gate (re-verified against a genuine authenticated cashier: `test_shift_belonging_to_a_different_authenticated_cashier_is_rejected`).

**`SaleFinalizeRequest`**: structural/shape validation only (`items`/`payments` array shape, `Money`/`Quantity` regex patterns, `PaymentMethod` enum), matching `CheckoutService`'s own docblock ("this class enforces domain/business invariants only, not request shape") — no business rule duplicated here. `client_expected_grand_total` and `items.*.override_reason` are accepted per the frozen schema but remain unused by `CheckoutService` today (a pre-existing, disclosed Stage 6C gap, not expanded in this pass — A6's scope is the auth wiring, not new checkout business logic).

**Response resources**: `SaleItemResource`/`PaymentResource`/`InvoiceSummaryResource`/`SaleDetailResource`, matching `openapi.yaml`'s `SaleDetail` schema field-for-field. `amount_tendered`/`change` are presentation-only derived values — `Sale` never persists them — computed at serialization time from the already-immutable `COMPLETED` sale's own payments, matching `stage-6c-sale-finalization.md`'s own words ("change computed server-side") without specifying a storage location. `void`/`refunds` are returned as `null`/`[]`: no void/refund endpoint exists yet, so a freshly finalized sale genuinely has neither.

**Error handling — no new HTTP-layer plumbing needed**: every exception `CheckoutService` can throw (`ShiftRequiredException`, `FiscalDayNotOpenException`, `ProductNotFoundException`, `ProductInactiveException`, `InvalidPaymentTotalException`, `InsufficientPaymentException`, `InvoiceSeriesExhaustedException`, `IdempotencyKeyReusedException`, `ConcurrencyConflictException`) is already a `DomainException` subclass, already rendered through `bootstrap/app.php`'s generic renderer since A1 first wired it. `IdempotencyKeyRequiredException` is new application code, but `IDEMPOTENCY_KEY_REQUIRED` (400, ADR-010) was already a frozen Stage 4 catalog entry with no implementing class until now — no amendment.

**A deliberate non-decision, carried forward rather than reopened**: `FiscalInstallationResolutionException`/`InventoryLocationResolutionException`/`TaxRegistrationResolutionException` remain uncaught by `SaleController`, surfacing as genuine unhandled 500s. This is not a new gap introduced here — each exception's own docblock already states the reasoning explicitly: these represent a store/terminal *setup* defect (no default inventory location, no active tax registration, no fiscal-installation mapping) that a properly configured store should never reach, not a business outcome a checkout caller should receive as a stable, programmatically-branchable HTTP error. Inventing a customer-facing code for "your store isn't configured correctly" would misrepresent an operational alerting condition as an expected API outcome. `PROJECT-MANIFEST.md`'s standing note that this "must close before production use" is best read as an operational-readiness reminder (every store needs its setup completed before going live), not a mandate to add a wire-level code — reopening that call was out of scope for A6 and not attempted.

**§13 matrix items re-verified against the real controller** (deferred at A5 pending exactly this controller's existence):
- **13** (User-store/terminal-store mismatch) — re-confirmed: `test_checkout_rejects_a_cross_store_terminal_credential` (403 `TERMINAL_NOT_ENROLLED`, A4's coherence check fires before `CheckoutService` is ever reached).
- **13a** (shift belongs to a different cashier) — now proven against a genuine authenticated cashier, not a trusted parameter: `test_shift_belonging_to_a_different_authenticated_cashier_is_rejected` (409 `SHIFT_REQUIRED`).
- **14/15** (request-body terminal/cashier-ID spoof) — `test_request_body_cannot_spoof_terminal_or_cashier_identity`: a spoofed `terminal_id`/`cashier_id` in the body has zero effect on the finalized Sale.
- **17** (checkout without terminal context) — `test_checkout_without_a_terminal_credential_is_rejected`, now proven against `saleFinalize` itself, not merely `terminalCurrent` as a stand-in.

**Files added**: `app/Domain/Exceptions/IdempotencyKeyRequiredException.php`, `app/Http/Controllers/SaleController.php`, `app/Http/Requests/SaleFinalizeRequest.php`, `app/Http/Resources/{SaleItemResource,PaymentResource,InvoiceSummaryResource,SaleDetailResource}.php`, `tests/Database/SaleFinalizationHttpTest.php` (7 methods).

**Files changed**: `routes/web.php` (the `POST /sales` route).

**Tests and totals**: Full regression: `tests/Unit` 102/474, `tests/Database` 208/736, `tests/Feature` 1/1, A1/A2/A3 suites individually re-verified, Stage 6A/6B concurrency, Stage 6C checkout (12/105) and checkout-concurrency (3/46) unchanged, `php artisan route:list --path=sales` confirms exactly one route, `scripts/validate-baselines.sh` 12/12, Pint clean. No baseline tag touched; no Stage 4 amendment.

**Module A (A0–A6) is complete.** All nine of §16's Stage 6C unblock criteria are now met and demonstrated by passing tests. `POST /sales`'s HTTP layer exists and is wired correctly; Stage 6C's own freeze as `stage-6c-baseline` is an explicit owner decision this document does not make unilaterally, per the project's standing practice of only sealing a stage on review. User-management CRUD (a separate feature the brief's Module A also names) was never part of the A0–A6 sequence and remains unbuilt.
