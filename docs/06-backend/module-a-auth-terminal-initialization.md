...
**RULINGS CLOSED. A0 CLOSED. A1 (human authentication/session foundation) GOVERNANCE-SEALED**, after two baseline reconstructions. **A2 (authorization/capability foundation) SEALED. A3 (terminal enrollment/credential verification) GOVERNANCE-SEALED**, after a third baseline reconstruction (see §19a). A4 through A6 have not started — no authoritative User+Terminal+Store composition, user-management endpoints, or Stage 6C HTTP integration exists yet.
...
**A3 sealing condition — all met**: Store scoping for terminal management is executable and tested; `terminalCurrent` does not bypass A4 (route deferred to testing-only, resolver fully implemented); token-claim single-use semantics proven under real concurrency and proven not to un-consume on a post-claim failure; `TERMINAL_NOT_ENROLLED`/`TERMINAL_REVOKED` remain distinct from `AUTHORIZATION_DENIED`; `ENROLLMENT_TOKEN_INVALID` now lives inside the canonical `stage-4-baseline` boundary; the full downstream baseline chain (5/6A/6B) is reconstructed and revalidated; every regression above is green. **A3 is GOVERNANCE-SEALED.**
