...
**Last updated:** 2026-09-18 — **A2 (authorization/capability foundation) implemented.** The fixed V1 role→capability model is now enforced via Laravel Gates: one `Gate::define()` per capability (`AppServiceProvider`), built entirely on A1's `RoleCapabilityCatalog` (no second mapping), Laravel's built-in `can:` middleware for whole-operation enforcement, and a new `AUTHORIZATION_DENIED` (403) exception renderer. Reconnaissance found the capability vocabulary already identical across `openapi.yaml`, `domain-model.md`, and the code catalog, so **no baseline reconstruction was needed or performed** — no tag moved. `PRICE_OVERRIDE`/`DISCOUNT_OVERRIDE`/`CASH_OUT` remain registered Gate abilities but are deliberately not applied as any whole-operation `can:` gate, matching their conditional/field-level nature in the frozen contract. Full detail in `docs/06-backend/module-a-auth-terminal-initialization.md` §18. **A3 (terminal enrollment) has not started**, nor has authoritative User+Terminal context, user management, or Stage 6C HTTP integration. Stage 6C's own status is unchanged: **checkout domain/service layer VERIFIED; `POST /sales` production readiness BLOCKED ON MODULE A.**
...
| **Frozen baselines** | Stage 1 (`stage-1-baseline` @ `d93a813`, unchanged), Stage 2 (`stage-2-baseline` @ `5a7382a`, unchanged), Stage 3 (`stage-3-baseline` @ `78a190c`, unchanged), Stage 4 (`stage-4-baseline` @ `f57e924` — **reconstructed twice**, A0 then the A1 `VALIDATION_FAILED` correction; originally `2313a16`), Stage 5 (`stage-5-baseline` @ `678d597` — reconstructed twice, was `4c16ee0`), Stage 6A (`stage-6a-baseline` @ `1c0e5c9` — reconstructed twice, was `47fa84d`), Stage 6B (`stage-6b-baseline` @ `d50abd7` — reconstructed twice, was `a667edc`). See "A0 baseline reconstruction" and "A1 baseline reconstruction" below for the full record of each. |
| **Next action** | Continue Module A: A3 (terminal enrollment + credential resolution), A4 (authoritative User+Terminal context). Only after A4 can Stage 6C's HTTP layer (`SaleController`/`FormRequest`/`routes/api.php`, A6) be built and Stage 6C frozen as `stage-6c-baseline`. |
| **Total ADRs** | 12 |
