...
**Last updated:** 2026-09-18 — **A1 (human authentication/session foundation) implemented.** `POST /auth/login`, `POST /auth/logout`, `GET /auth/me` now exist against the reconstructed (post-A0) contract: standard Laravel database-backed session auth, `EnsureUserIsActive` revalidation on every protected request, named `login` rate limiter → `RATE_LIMITED`, and the `UserSummaryResource`/`RoleCapabilityCatalog` pair that centralizes the frozen `UserSummary` shape. No baseline tag moved. Full detail in `docs/06-backend/module-a-auth-terminal-initialization.md` §17. **A2 (RBAC/capability enforcement) has not started**, nor has terminal enrollment, authoritative context, user management, or Stage 6C HTTP integration. Stage 6C's own status is unchanged: **checkout domain/service layer VERIFIED; `POST /sales` production readiness BLOCKED ON MODULE A.** A0's baseline-reconstruction record (Stage 4/5/6A/6B rebuilt on the two approved amendments) remains below under "A0 baseline reconstruction," unchanged by this pass.
