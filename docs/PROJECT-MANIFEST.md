...
**Last updated:** 2026-09-18 — **A1 (human authentication/session foundation) SEALED.** `POST /auth/login`, `POST /auth/logout`, `GET /auth/me` exist against the reconstructed (post-A0) contract: standard Laravel database-backed session auth, `EnsureUserIsActive` revalidation on every protected request, named `login` rate limiter → `RATE_LIMITED`, the `UserSummaryResource`/`RoleCapabilityCatalog` pair centralizing the frozen `UserSummary` shape, structural login-validation failures rendering as the frozen `VALIDATION_FAILED` (422) envelope, and a proven CSRF-bootstrap path for a fresh browser (the existing `GET /` application-shell route already sets `XSRF-TOKEN`). No baseline tag moved. Full detail in `docs/06-backend/module-a-auth-terminal-initialization.md` §17. **A2 (RBAC/capability enforcement) has not started**, nor has terminal enrollment, authoritative context, user management, or Stage 6C HTTP integration. Stage 6C's own status is unchanged: **checkout domain/service layer VERIFIED; `POST /sales` production readiness BLOCKED ON MODULE A.** A0's baseline-reconstruction record (Stage 4/5/6A/6B rebuilt on the two approved amendments) remains below under "A0 baseline reconstruction," unchanged by this pass.
...
| **Frozen baselines** | Stage 1 (`stage-1-baseline` @ `d93a813`, unchanged), Stage 2 (`stage-2-baseline` @ `5a7382a`, unchanged), Stage 3 (`stage-3-baseline` @ `78a190c`, unchanged), Stage 4 (`stage-4-baseline` @ `d8f05f9` — **reconstructed by A0**, was `2313a16`), Stage 5 (`stage-5-baseline` @ `d5fdf87` — **reconstructed by A0**, was `4c16ee0`), Stage 6A (`stage-6a-baseline` @ `6250574` — **reconstructed by A0**, was `47fa84d`), Stage 6B (`stage-6b-baseline` @ `f4601ca` — **reconstructed by A0**, was `a667edc`). See "A0 baseline reconstruction" below for the full record. |
...
**Superseded (all previous reconciliation attempts, reachable via `git reflog`/`backup/pre-final-baseline-reconcile` or, for the A0 reconstruction, `backup/pre-module-a-a0`, no longer referenced by any tag):** the original interleaved history (`6da07bd`, `b7ef8fd`, `c8a8bbe` (as a Stage-2-ancestor-containing tag target), `40aaebd`/`515e2db`, `21bf0c2`/`8c8fe32`, `47baddf`/`48d173e`), the rejected first InvoiceSeries reconciliation (`d1bfaa7`, `f4ca1d9`), the rejected two-part reconciliation (`e1fb82a`, `f036da9`, `570dd75`, `228e033`, `ceab7d4`), `b335a7a` (the first `stage-6b-baseline` candidate) and `3fb7c70` (the original ungrouped governance commit), and — from the A0 reconstruction — the pre-A0 `stage-4-baseline`/`stage-5-baseline`/`stage-6a-baseline`/`stage-6b-baseline` targets (`2313a16`, `4c16ee0`, `47fa84d`, `a667edc`) and the pre-A0 `main` tip (`73587d1`), all still reachable via `backup/pre-module-a-a0`. `c8a8bbe` and `515e2db` remain historically meaningful as "the commit whose *content* Stage 3/4 were verified against," but are no longer any tag's target.
