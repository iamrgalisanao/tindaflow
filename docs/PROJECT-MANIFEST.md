...
**Last updated:** 2026-09-18 — **A1 (human authentication/session foundation) GOVERNANCE-SEALED**, after a second baseline reconstruction. `POST /auth/login`, `POST /auth/logout`, `GET /auth/me` exist against the twice-reconstructed contract: standard Laravel database-backed session auth, `EnsureUserIsActive` revalidation on every protected request, named `login` rate limiter → `RATE_LIMITED`, the `UserSummaryResource`/`RoleCapabilityCatalog` pair centralizing the frozen `UserSummary` shape, structural login-validation failures rendering as the frozen `VALIDATION_FAILED` (422) envelope, and a proven CSRF-bootstrap path for a fresh browser (the existing `GET /` application-shell route already sets `XSRF-TOKEN`). `VALIDATION_FAILED` was initially added as a forward edit to `error-catalog.md`, which the owner correctly identified as the same class of frozen-contract change A0 already established requires reconstruction rather than a forward commit — see "A1 baseline reconstruction" below for that correction's full record. `stage-4-baseline`/`stage-5-baseline`/`stage-6a-baseline`/`stage-6b-baseline` have moved again (all four now carry `VALIDATION_FAILED` at the Stage 4 boundary). Full detail in `docs/06-backend/module-a-auth-terminal-initialization.md` §17. **A2 (RBAC/capability enforcement) has not started**, nor has terminal enrollment, authoritative context, user management, or Stage 6C HTTP integration. Stage 6C's own status is unchanged: **checkout domain/service layer VERIFIED; `POST /sales` production readiness BLOCKED ON MODULE A.**
...
| **Next action** | Continue Module A: A2 (RBAC/capability `Gate` checks), A3 (terminal enrollment + credential resolution), A4 (authoritative User+Terminal context). Only after A4 can Stage 6C's HTTP layer (`SaleController`/`FormRequest`/`routes/api.php`, A6) be built and Stage 6C frozen as `stage-6c-baseline`. |
| **Total ADRs** | 12 |
